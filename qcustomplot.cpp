...
void QCPGraph::setData(QVector<double> &x, QVector<accData> &y)
{
    mData->clear();
    ...
    {
        newDataX.key = x[i];
        newDataX.value = y[i].accX;

        mData->insertMulti(newDataX.key, newDataX);
    }

    for (int i=0; i<n; ++i)
    {
        newDataY.key = x[i];
        newDataY.value = y[i].accY;

        mData->insertMulti(newDataY.key, newDataY);
    }

    for (int i=0; i<n; ++i)
    {
        newDataZ.key = x[i];
        newDataZ.value = y[i].accZ;

        mData->insertMulti(newDataZ.key, newDataZ);
    }
}

void QCPGraph::setData(QVector<double> &x, QVector<gyrData> &y)
{
    mData->clear();
    int n = x.size();
    n = qMin(n, y.size());
    ...
    {
        newDataZ.key = x[i];
        newDataZ.value = y[i].gyrZ;

        mData->insertMulti(newDataZ.key, newDataZ);
    }
}

void QCPGraph::setData(QVector<double> &x, QVector<magData> &y)
{
    mData->clear();
    int n = x.size();
    n = qMin(n, y.size());
